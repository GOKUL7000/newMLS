...
export default function PermitDuePage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    const { data: vehicles, error } = await supabase
      .from('vehicles')
      .select('id, vehicle_no, brand, model, permit_valid_up_to, driver_id, status')
      .eq('deleted', false)
      .neq('status', 'Inactive');
    ...
    const mapped: Row[] = (vehicles || []).map(v => {
      const days = daysLeft(v.permit_valid_up_to);
      return {
        id: v.id,
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      <Topbar title="Permit Due" breadcrumbs={[{ label: 'Fleet' }, { label: 'Permit Due' }]} />
      <div className="flex-1 overflow-y-auto p-4 space-y-4">

        {/* Stats */}
        <div className="grid grid-cols-5 gap-3">
          {[
            { label: 'Expired',            value: stats.expired,  color: 'text-red-700',    bg: 'bg-red-50 border-red-100',   sub: 'Permit Expired' },
        ...
        <div className="bg-white rounded-xl p-5 shadow-sm border border-gray-100">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h3 className="text-[13px] font-semibold text-gray-800">Permit Due — All Vehicles</h3>
              <p className="text-[10px] text-gray-400 mt-0.5">Sorted by urgency. Reads permit_valid_up_to from vehicles table.</p>
            </div>
            ...
            <table className="w-full text-[11px]">
              <thead>
                <tr className="text-gray-400 border-b border-gray-100 bg-gray-50">
                  {['#', 'Vehicle No.', 'Brand / Model', 'Driver', 'Permit Expiry', 'Days Left', 'Alert', ''].map((h, i) => (
