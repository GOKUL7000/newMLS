...
import { Truck, FileText, LogOut, MapPin, ChevronDown, ChevronUp, AlertCircle, Package } from 'lucide-react';
...
type Trip = {
  ...
  supplier_amount: number; status: string; lr_no: string | null;
...
export default function SupplierPortalPage() {
  ...
  async function init() {
    ...
    if (!s || s.role !== 'supplier') { router.push('/portal/login'); return; }
    setSession(s);
      ...
      <main className="max-w-4xl mx-auto px-4 py-6 space-y-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {[
            ...
            { label: 'Active Trips', value: String(trips.filter(t => !['Settled','Pending'].includes(t.status)).length), color: 'text-amber-600', bg: 'bg-amber-50' },
            { label: 'Total Billed', value: fmt(invoices.reduce((s,i)=>s+i.amount,0)), color: 'text-purple-600', bg: 'bg-purple-50' },
            { label: 'Outstanding', value: fmt(totalOutstanding), color: totalOutstanding > 0 ? 'text-red-600' : 'text-emerald-600', bg: totalOutstanding > 0 ? 'bg-red-50' : 'bg-emerald-50' },
                ...
                <div key={trip.id} className="bg-white rounded-xl border border-gray-200 overflow-hidden">
                  <button onClick={() => setExpandedTrip(expandedTrip === trip.id ? null : trip.id)}
                    ...
                    <div className="text-left">
                      ...
                      {trip.vehicles?.registration_no && <p className="text-xs text-gray-400 mt-0.5">Vehicle: {trip.vehicles.registration_no}</p>}
                    </div>
                    <div className="flex items-center gap-3">
                      <span className="text-sm font-semibold text-gray-900">{fmt(trip.supplier_amount || 0)}</span>
                  ...
                  {expandedTrip === trip.id && (
                    <div className="px-5 pb-4 border-t border-gray-100 bg-gray-50">
                      <div className="grid grid-cols-2 md:grid-cols-3 gap-4 pt-3">
                        ...
                        <div><p className="text-xs text-gray-400">Supplier Amount</p><p className="text-sm font-bold text-gray-900 mt-0.5">{fmt(trip.supplier_amount || 0)}</p></div>
                      </div>
                    </div>
                  )}
                </div>
